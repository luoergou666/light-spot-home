...
class Media {
  ...
  loadImage(texture: Texture) {
    ...
    img.onerror = () => {
      ...
      fallbackImg.src = 'https://picsum.photos/256/256?grayscale';
      fallbackImg.onload = () => {
...
class App {
  ...
  lastAutoScrollTime: number = 0;
  lastDebugTime: number = 0;
  ...
  onImageLoaded() {
    ...
    if (this.loadedImagesCount >= this.totalImagesCount && this.onImagesLoaded) {
      this.onImagesLoaded();
    }
  }
  ...
  createMedias(
    ...
    this.totalImagesCount = this.mediasImages.length;
  ...
  update() {
    ...
    if (this.autoScroll && !this.isDown) {
      ...
      if (currentTime - this.lastAutoScrollTime > 16) { // 约60fps
        this.scroll.target -= this.autoScrollSpeed;
        this.lastAutoScrollTime = currentTime;
      }
    }
...
onMounted(() => {
  ...
  app = new App(containerRef.value, {
    ...
    onImagesLoaded: () => emit('images-loaded')
  });
});
...
// 单独监听font属性变化，使用防抖避免频繁更新
const fontUpdateTimeout = ref(null);
watch(
  () => props.font,
  (newFont) => {
    if (fontUpdateTimeout.value) {
      clearTimeout(fontUpdateTimeout.value);
    }
    fontUpdateTimeout.value = setTimeout(() => {
